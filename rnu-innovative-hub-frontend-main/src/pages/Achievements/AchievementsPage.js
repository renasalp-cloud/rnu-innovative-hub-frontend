...
function AchievementsPage() {
  ...
  setItemsState((prev) => ({
    ...prev,
    [currentKey]: [newItem, ...(prev[currentKey] || [])],
  }));
  ...
  try {
    //  2. BACKEND SYNC
    const saved = await studentApi.addItem(email, currentKey, newItem);

    //  3. REPLACE TEMP ITEM WITH REAL ONE (ID FROM BACKEND)
    if (saved?.item) {
      setItemsState((prev) => ({
        ...prev,
        [currentKey]: prev[currentKey].map((it) =>
          it.id === tempId ? saved.item : it
        ),
      }));
    }
    ...
    setItemsState((prev) => ({
      ...prev,
      [currentKey]: prev[currentKey].filter((it) => it.id !== tempId),
    }));
